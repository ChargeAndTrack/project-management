...
italiano, e si è riscontrato che nessuno di essi offre soluzioni digitali per la ricerca e l'utilizzo delle
colonnine di ricarica. Inoltre, è stata condotta un'indagine tra i clienti e si è riscontrato che il 70\% di essi
sarebbe interessato ad una soluzione digitale. In particolare, ha trovato un alto interesse la possibilità di
...
\begin{itemize}
    ...
    dell'applicazione, considerando almeno 100 voti. La valutazione viene richiesta a tutti gli utenti entro un anno
    dal rilascio dell'applicazione, tramite indagine interna all'applicazione, e se ne calcolerà la media al termine
    della raccolta.
\end{itemize}
...
\begin{itemize}
    ...
    database documentale per la sua flessibilità e scalabilità, che si adatta meglio alle esigenze dell'applicazione,
    ...
    ricarica, si è deciso di utilizzare un codice QR, in quanto il meccanismo di lettura è già presente nel software
    delle colonnine di ricarica;
    \item in riferimento all'obiettivo 1, dato che l'autenticazione degli utenti avverrà tramite email e password,
    il meccanismo di autenticazione a due fattori sarà implementato tramite l'invio di un codice temporaneo via email.
\end{itemize}
