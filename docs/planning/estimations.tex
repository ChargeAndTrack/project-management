...
\section{Stima delle quantità di lavoro}
Per ogni attività della WBS è stata effettuata una stima della quantità di lavoro necessaria per completarla,
utilizzando una tecnica \textit{consensus-based}. Nello specifico, per i task con una bassa complessità e grado di
incertezza è stato utilizzato il metodo \textit{Three-Point}, basandosi sulle tre stime (ottimistica (O), pessimistica
(P) e più probabile (M)) di una figura esperta in quell'attività, per poi calcolare la stima finale come media
ponderata delle tre stime, utilizzando la formula:
\begin{center}
    $E = \frac{O + 4M + P}{6}$
\end{center}
...
Il team di sviluppo è composto da 10 membri, di cui 5 sono specializzati nel frontend e 5 nel backend, ed entrambi 
gruppi sono organizzati in maniera orizzontale e sono composti da sviluppatori senior e junior. Per questa ragione, i
task saranno assegnati durante l'esecuzione del progetto in base alla disponibilità dei membri del team. Tuttavia, è
stata effettuata una stima del numero di risorse (sviluppatori) necessarie per ogni task.
Nella \Cref{table:resources_estimated} sono riportati alcuni esempi di stime delle risorse.

\begin{table}[!htb]
    \centering
    \begin{tabular}{|c|c|c|}
        \hline
        \textbf{Attività} & \textbf{Categoria} & \textbf{Numero di risorse} \\
        \hline
...
Nella \Cref{table:duration_estimated} sono riportati alcuni esempi di stime delle durate.

\begin{table}[!htb]
    \centering
    \begin{tabular}{|c|c|}
        \hline
        \textbf{Attività} & \textbf{Durata (giorni)} \\
        \hline
        1.2.1    & 3  \\
        \hline
        4.2.2    & 5  \\
...
Nella \Cref{table:cost_estimated} sono riportati alcuni esempi di stime dei costi, in cui il costo di ogni attività è
calcolato come il prodotto tra il costo delle risorse stimate (differenziato per tipo di risorsa) e la durata stimata
dell'attività.
...
\begin{table}[!htb]
    ...
    \begin{tabular}{|c|c|}
        \hline
        \textbf{Attività} & \textbf{Costo (euro)} \\
        \hline
...
Il piano di pagamento proposto prevede un anticipo del 20\% (36.000 euro) alla firma del contratto, e il restante
suddiviso in:
\begin{itemize}
    \item 25\% (45.000 euro) in corrispondenza della milestone 1;
    \item 25\% (45.000 euro) in corrispondenza della milestone 2;
    \item 30\% (54.000 euro) alla consegna del progetto.
\end{itemize}
